...
namespace SSS
{
    public partial class PurchaseTypeMaster : Form
    {
        ...
        public PurchaseTypeMaster(bool chk, string strName)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            BindDataGrid();
        ...
        private void EnableAllControl()
        {
            ...
            grpItemprice.Enabled = grpMRP.Enabled = grpRegion.Enabled = grpTTypeBox.Enabled = grpGST.Enabled = true;
        ...
        private void DisableAllControl()
        {
            ...
            dgrdName.Enabled = true;
             grpItemprice.Enabled = grpMRP.Enabled = grpRegion.Enabled = grpTTypeBox.Enabled =grpGST.Enabled = false;
        }

        private void ClearAllText()
        {
            txtTaxName.Text = txtIGSTAccount.Text = txtSGSTAccount.Text = txtSearch.Text = lblMsg.Text =lblCreatedBy.Text= "";
            rdoTaxableVoucherwise.Checked = rdoLocal.Checked = rdoMRPNo.Checked = rdoTaxInNo.Checked = rdoGSTNo.Checked= true;
        ...
        private void BindDataGrid()
        {
            try
            {
                objTable = dba.GetDataTable("Select ID,TaxName,Region from SaleTypeMaster Where SaleType='PURCHASE'  Order by TaxName");
                if (objTable != null)
                {
                    dgrdName.Rows.Clear();
        ...
        private void BindAllDetails(DataGridViewRow rows)
        {
            try
            {
                ...
                {
                    DataTable _dt = dba.GetDataTable("Select *,dbo.GetFullName(TaxAccountIGST) TaxAIGST,dbo.GetFullName(TaxAccountSGST) TaxASGST,(Select ISNULL(Count(*),0) from PurchaseRecord Where TaxLedger=TaxName)_Count from SaleTypeMaster Where  SaleType='PURCHASE' and  TaxName='" + strTaxType + "' ");
                    if (_dt.Rows.Count > 0)
                    {
                        ...
                        else if (Convert.ToString(row["TaxationType"]) == "COMPOSITION")
                            rdoComposition.Checked = true;
                        ...
                        else if (Convert.ToString(row["TaxationType"]) == "UNREG")
                            rdoUnReg.Checked = true;
                        else
        ...
        private bool CheckAvailability()
        {
            try
            {
                if (txtTaxName.Text != "")
                {
                    if (btnAdd.Text == "&Save")
                    {
                        DataTable MyTable = dba.GetDataTable("Select TaxName from SaleTypeMaster Where SaleType='PURCHASE' and TaxName ='" + txtTaxName.Text + "'");
                    ...
                    {
                        DataTable MyTable = dba.GetDataTable("Select TaxName from SaleTypeMaster Where SaleType='PURCHASE' and TaxName ='" + txtTaxName.Text + "' and ID !=" + lblId.Text + "");
                        if (MyTable.Rows.Count > 0)
                        {
                            lblMsg.Text = "Sorry ! Tax Name : " + txtTaxName.Text + " already exist ! ";
                            lblMsg.ForeColor = Color.Red;
        ...
        private void SaveRecord()
        {
            try
            {
                ...
                string strQuery = " if not exists (Select TaxName from [dbo].[SaleTypeMaster] Where SaleType='PURCHASE' and TaxName='" + txtTaxName.Text + "') begin INSERT INTO [dbo].[SaleTypeMaster] ([TaxName],[Region],[TaxationType],[EcommType],[TaxOnMRP],[TaxIncluded],[EcommPortalName],[TaxAccountIGST],[TaxAccountSGST],[Other],[SkipGST],[InsertStatus],[UpdateStatus],[CreatedBy],[UpdatedBy],[SaleType],[IGSTTaxRate],[SGSTTaxRate]) VALUES "
                                            + " ('"+txtTaxName.Text+"','"+GetRegion()+"','"+GetTaxationType()+"',0,'"+rdoMRPYes.Checked+"','"+rdoTaxInYes.Checked+"','','"+ strIGSTAccount + "','"+ strSGSTAccount + "','','"+rdoGSTYes.Checked+"', 1,0,'"+MainPage.strLoginName+ "','','PURCHASE'," + dba.ConvertObjectToDouble(txtIGSTRate.Text) + "," + dba.ConvertObjectToDouble(txtSGSTRate.Text) + ") end ";
        ...
        private string GetTaxationType()
        {
            if (rdoTaxableVoucherwise.Checked)
                ...
                return "ZERORATED";
            else if (rdoComposition.Checked)
                return "COMPOSITION";
            else if (rdoUnReg.Checked)
                return "UNREG";
            else
                return "NONGST";
        }
                ...
                if (lblId.Text != "" && row != null)
                {
                    if (Convert.ToString(row.Cells["id"].Value) == lblId.Text)
                    {
                        ...
                        if (strOldName != "")
                        {
                            ...
                            object objStatus = DataBaseAccess.ExecuteMyScalar("Select InsertStatus from [dbo].[SaleTypeMaster] Where SaleType='PURCHASE' and TaxName='" + strOldName + "' ");

                            string strQuery = " Update [dbo].[SaleTypeMaster] SET [TaxName]='" + txtTaxName.Text + "',[Region]='" + GetRegion() + "',[TaxationType]='" + GetTaxationType() + "',[TaxOnMRP]='" + rdoMRPYes.Checked + "',[TaxIncluded]='" + rdoTaxInYes.Checked + "',[TaxAccountIGST]='" + strIGSTAccount + "',"
                                            + " [TaxAccountSGST]='" + strSGSTAccount + "',[Other]='',[SkipGST]='" + rdoGSTYes.Checked + "',[UpdateStatus]=1,[UpdatedBy]='" + MainPage.strLoginName + "',[IGSTTaxRate]=" + dba.ConvertObjectToDouble(txtIGSTRate.Text) + ",[SGSTTaxRate]=" + dba.ConvertObjectToDouble(txtSGSTRate.Text) + " Where SaleType='PURCHASE' and TaxName='" + strOldName + "' ";
                            if (strOldName != txtTaxName.Text)
                                strQuery += " Update PurchaseRecord Set TaxLedger='" + txtTaxName.Text + "' WHere TaxLedger='" + strOldName + "' ";
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (lblId.Text != "" && row != null)
                        {
                            if (Convert.ToString(row.Cells["id"].Value) == lblId.Text)
                            {
                                ...
                                {
                                    object objStatus = DataBaseAccess.ExecuteMyScalar("Select InsertStatus from [dbo].[SaleTypeMaster] Where SaleType='PURCHASE' and TaxName='" + strOldName + "' ");
                                    strQuery = "Delete from [dbo].[SaleTypeMaster] Where SaleType='PURCHASE' and TaxName='" + strOldName + "' ";
        ...
        private void PurchaseTypeMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
            {
