...
namespace SSS
{
    public partial class CartonSizeMaster : Form
    {
        DataBaseAccess dba;
        DataTable objTable;
        ...
        public CartonSizeMaster(bool chk,string strName)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            BindDataGrid();
        ...
        private void BindDataGrid()
        {
            try
            {
                objTable = dba.GetDataTable("Select * from CartoneSize Order by CSize");
                if (objTable != null)
                {
                    dgrdName.Rows.Clear();
                    if (objTable.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in objTable.Rows)
                        {
                            ...
                            dgrdName.Rows[rowIndex].Cells["itemName"].Value = dr["CSize"];
                            dgrdName.Rows[rowIndex].Cells["packingAmt"].Value = dr["PackingAmt"];
                            dgrdName.Rows[rowIndex].Cells["packingType"].Value = dr["PackingType"];
        ...
        private void BindAllDetails(DataGridViewRow row)
        {
            try
            {
                ...
                txtPackingAmt.Text = Convert.ToString(row.Cells["packingAmt"].Value);
                txtPackingType.Text = Convert.ToString(row.Cells["packingType"].Value);
        ...
        private void SaveRecord()
        {
            try
            {
               // string Insertquery = "Insert into UnitMaster (UnitName,FormalName,DecimalPoint) values('" + txtName.Text + "','" + txtFormalName.Text + "','"+txtDecimalPoint.Text+"')";
                int rd = dba.SaveNewCartoneSize(txtName.Text, txtPackingAmt.Text,txtPackingType.Text);
                if (rd > 0)
                {
        ...
        {
            txtName.ReadOnly = txtPackingAmt.ReadOnly= false;
        ...
        {
            txtName.ReadOnly = txtPackingAmt.ReadOnly = true;
        ...
        {
            txtName.Text =  txtSearch.Text = lblMsg.Text =  txtPackingAmt.Text= txtPackingType.Text= ""; 
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {    
                if (btnAdd.Text == "&Add" && btnEdit.Text == "&Edit" && lblId.Text!="")
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        int i = dba.ExecuteMyQuery("Delete from CartoneSize Where ID=" + lblId.Text + "");
                        if (i > 0)
                        {
        ...
        private void UpdateRecord()
        {
            try
            {
                ...
                if (lblId.Text != "" && row != null)
                {
                    if (Convert.ToString(row.Cells["id"].Value) == lblId.Text)
                    {
                        ...
                        int count = dba.UpdateCartoneSize(txtName.Text, strOldName, txtPackingAmt.Text,txtPackingType.Text);
                        if (count > 0)
                        {
            ...
            {
                string[] StrReport = { "Error Occur on Update Record in Carton Master.", ex.Message };
            }
        }
        ...
        {           
            try
            {
                if (txtName.Text != "")
                {
                    if (btnAdd.Text == "&Save")
                    {
                        DataTable MyTable = dba.GetDataTable("Select CSize from CartoneSize Where CSize ='" + txtName.Text + "'");
                        if (MyTable.Rows.Count > 0)
                        {
                            lblMsg.Text = "Sorry ! Carton Size : " + txtName.Text + " already exist ! ";
                ...
                {
                    lblMsg.Text = "Sorry ! Carton Size can't be blank ! ";
                    lblMsg.ForeColor = Color.Red;
                    txtName.Focus();
                    return false;
                }
        ...
        private void SearchResult()
        {
            try
            {
                if (objTable != null)
                {
                    if (txtSearch.Text!="")
                    {
                        DataRow[] row = objTable.Select(String.Format("CSize Like ('%" + txtSearch.Text + "%')"));
                        if (row.Length > 0)
                        {
        ...
        private void txtname_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if ((Char.IsWhiteSpace(e.KeyChar) && (txtName.Text.Length < 1 || (txtName.Text.Length <= txtName.SelectionLength || txtName.SelectionStart == 0))))
                {
                    e.Handled = true;
                }
            }
        ...
        private void CartonSizeMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
