...
namespace SSS
{
    public partial class MarketerMaster : Form
    {
        DataBaseAccess dba;
        public string StrAgentName = "", __strName="";
        ...
        public MarketerMaster(int count, string strName)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            intStatus = count;
        ...
        private void BindRecordWithControls(DataRow dr)
        {
            ...
            txtMobileNo.Text = Convert.ToString(dr["MobileNoI"]);
            txtOrderCode.Text = Convert.ToString(dr["OrderCode"]);
            txtMCode.Text = Convert.ToString(dr["MCode"]);
        ...
        private void BindListBoxDetail()
        {
            ...
            {
                string[] StrReport = { "Error Occur on Binding Details Name from Marketer Master", ex.Message };
        ...
        private void BindSearchListBox(TextBox txtBox)
        {
            ...
            {
                string[] StrReport = { "Error Occur on Binding Name from Marketer Master", ex.Message };
                dba.CreateErrorReports(StrReport);
            }
        }
        ...
        public int CheckMCodeAvailability(string strName)
        {
            int count = 0;
            try
            {
                string strSelectedName = "", strQuery = "",strMQuery="";
                if (btnEdit.Text == "&Update")
                    strSelectedName = Convert.ToString(lboxAgentName.SelectedItem);
                if (txtMobileNo.Text != "")
                    strMQuery = " OR MobileNoI='" + txtMobileNo.Text + "' ";
                strQuery = "Select Count(*) from Marketer where (MCode='" + strName + "' "+ strMQuery+") and MarketerName!='" + strSelectedName + "'  ";
                object objValue = "0";
                if (MainPage.strLiveDataBaseIP != "")
                    objValue = NetDBAccess.ExecuteMyScalar(strQuery);
                else
                    objValue = DataBaseAccess.ExecuteMyScalar(strQuery);
                return dba.ConvertObjectToInt(objValue);
            }
            catch { }
            return count;
        }
        ...
        private bool CheckAvailability()
        {
            ...
            try
            {
                if (txtMCode.Text != "")
                {
                    int check = CheckMCodeAvailability(txtMCode.Text);
                    if (check < 1)
                    {
                        lblMCode.Text = "MCode/MobileNo is available Please proceed.";
                        lblMCode.ForeColor = Color.Green;
                        lblMCode.Visible = true;
                        chk = true;
                    }
                    else
                    {
                        lblMCode.Text = "MCode/MobileNo is already exist ! ";
                        lblMCode.ForeColor = Color.Red;
                        lblMCode.Visible = true;
                        txtMCode.Focus();
                        chk = false;
                    }
            ...
            {
                string[] strReport = { "Error occurred in check Availability ", ex.Message };
                dba.CreateErrorReports(strReport);
                txtMCode.Focus();
            }
            return chk;
        ...
        public bool CheckDuplicacy()
        {
            ...
            {
                lblMsg.Text = "Marketer Name is required ! ";
                lblMsg.ForeColor = Color.Red;
                txtMarketerName.Focus();
                return false;
            }
        ...
        private void SaveAgentMaster()
        {
            try
            {
                if (btnAdd.Text == "&Add")
                {
                    ...
                    EnableAllControls();
                    txtMarketerName.Focus();
                    txtOrderCode.Text = Convert.ToString(DataBaseAccess.ExecuteMyScalar("select OrderCode  from companysetting where Companyname='" + MainPage.strCompanyName + "'"));
                }
                else if (btnAdd.Text == "&Save")
                {
                    if (CheckDuplicacy() && CheckAvailability())
                    {
                        ...
                        if (dr == DialogResult.Yes)
                        {
                            ...
                            record[9] = txtOrderCode.Text;
                            record[10] = txtMCode.Text;
                          ...
                          //  int result = dba.ExecuteQuery(strQuery);
                            if (result > 0)
                            {
                                ...
                                if (intStatus > 0)
                                {
                                    StrAgentName = txtMarketerName.Text;
                                    this.Close();
                                }
                                lblMsg.Text = "";
            ...
            {
                string[] strReport = { "Error Saving Record in Marketer Master", ex.Message };
                dba.CreateErrorReports(strReport);
            }
        }
        ...
        public void UpdateAgent()
        {
            try
            {
                ...
                else if (CheckDuplicacy() && CheckAvailability())
                {

                    DialogResult dr = MessageBox.Show("Are you sure you want to update marketer details  ? ", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (dr == DialogResult.Yes)
                    {
                        ...
                        if (strSelectedAgent != "")
                        {
                            ...
                            record[10] = txtOrderCode.Text;
                            record[11] = txtMCode.Text;
                            ...
                            if (result > 0)
                            {
                                ...
                                btnEdit.Text = "&Edit"; lblMCode.Visible = false;
                                BindListBoxDetail();
                            }
                            else
                            {
                            ...
                            MessageBox.Show("Please select marketer name in marketer list !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch (Exception ex)
            {
                string[] strReport = { "Error Updateing Record in Marketer Master", ex.Message };
                dba.CreateErrorReports(strReport);
            }
        }
        ...
        public void DeleteAgentRecord()
        {
            try
            {
                ...
                {
                    if (MessageBox.Show("Are you sure you want to delete this marketer details ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        string _Query = " Delete From Marketer Where MarketerName='" + strSelectedAgentName + "' ";
                        int temp = dba.ExecuteMyQuery(_Query);
                        if (temp == 1)
                        {
                            MessageBox.Show("Thank you ! marketer deleted  successfully ..", "Message", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                            btnAdd.Text = "&Add";
                            btnEdit.Text = "&Edit";
                            BindListBoxDetail();
                        }
            ...
            {
                string[] strReport = { "Error Deleting Record in Marketer Master", ex.Message };
                dba.CreateErrorReports(strReport);
            }
        }
        ...
        private void MarketerMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
