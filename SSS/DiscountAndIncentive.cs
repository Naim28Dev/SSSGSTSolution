...
namespace SSS
{
    public partial class DiscountAndIncentive : Form
    {
        DataBaseAccess dba;
        ...
        private void SetDataName()
        {
            ...
            {
                lblSelect.Text = "Brand :";
                txtData.Text = "";
            }
            //else if (rdoBySupplier.Checked)
            //{
        ...
        private void txtData_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                string strData = "", strHeader = "";
                if (rdoByBrand.Checked)
                {
                    strData = "BRANDNAME";
                    strHeader = "SELECT BRAND NAME";
                }
                else if (rdoByBarcode.Checked)
                {
                    strData = "BARCODE";
                    strHeader = "SELECT BAR CODE";
                }
                else if (rdoByItem.Checked)
                {
                    strData = "ITEM_CODE";
                    strHeader = "SELECT ITEM NAME";
                }
                //else if (rdoBySupplier.Checked)
                //{
                //    strData = "PURCHASEPARTY";
                ...
                if (strData != "")
                {
                    ...
                    {
                        SearchData objSearch = new SearchData(strData, strHeader, e.KeyCode);
                        objSearch.ShowDialog();
                        txtData.Text = objSearch.strSelectedData;
                    }
                }
            }
            catch { }
        ...
        private void DiscountAndIncentive_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private bool ValidateControl()
        {
            if (rdoByMRP.Checked)
            {
                ...
                if (dToMRP == 0)
                {
                    MessageBox.Show("Sorry ! MRP range must be enter  !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtToMRP.Focus();
                    return false;
                }
            }
            ...
            {
                MessageBox.Show("Sorry ! Entry can't blank !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtData.Focus();
                return false;
            }
        ...
        private void BindData()
        {
            try
            {
                string strQuery = "Select * ,Convert(varchar,StartDate,103)SDate,Convert(varchar,EndDate,103)EDate,Convert(varchar,Date,103)_Date  from [dbo].[IncentiveDetails] Order by Date desc ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        dgrdDetail.Rows[rowIndex].Cells["data"].Value = row["InentiveValue"];
                        ...
                        dgrdDetail.Rows[rowIndex].Cells["editAction"].Value = "Edit";
                        dgrdDetail.Rows[rowIndex].Cells["deleteAction"].Value = "Delete";
                        rowIndex++;
        ...
        {
            strOldBranchCode = strOldIncentiveValue = strOldIncentiveOn = "";
            rdoByBrand.Checked = true;
            txtData.Text = "";
            chkDate.Checked = chkMRP.Checked = false;
        }
        ...
        private void dgrdDetail_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    if (e.ColumnIndex == 10)
                    {
                        SetDataInControl(dgrdDetail.CurrentRow);
                    }
                    else if (e.ColumnIndex == 11)
                    {
                        DeleteRecord(dgrdDetail.CurrentRow);
                    }
                }
            }
            catch { }
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (ValidateControl())
                {
                    DialogResult result = MessageBox.Show("Are you sure you want to submit record ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void SubmitData()
        {
            ...
            strQuery += " if exists (Select IncentiveOn from IncentiveDetails Where IncentiveOn='" + strOldIncentiveOn + "' and InentiveValue='" + strOldIncentiveValue + "' and BranchCode='" + strOldBranchCode + "') begin "
                            + " UPDATE[dbo].[IncentiveDetails] SET  [IncentiveOn] ='" + strIncOn + "',[InentiveValue] = '" + txtData.Text + "',[IncentiveType] ='" + strIncType + "' ,[IncentivePer] =" + dValue + " ,[FilterByDate] ='" + chkDate.Checked.ToString() + "' ,[StartDate] =" + strStartDate + " ,[EndDate] =" + strEndDate + " ,[FilterByMRP] ='" + chkMRP.Checked.ToString() + "' ,[StartMRP] =" + dStartMRP + ",[EndMRP] =" + dEndMRP + " ,[Date] =DATEADD(MINUTE,30,DATEADD(hh,5,GETUTCDATE())) ,[UpdatedBy] = '" + MainPage.strLoginName + "',[UpdateStatus] = 0 WHERE IncentiveOn='" + strOldIncentiveOn + "' and InentiveValue='" + strOldIncentiveValue + "' and BranchCode='" + strOldBranchCode + "' end else begin  "
                            + " INSERT INTO [dbo].[IncentiveDetails] ([BranchCode],[IncentiveOn],[InentiveValue],[IncentiveType],[IncentivePer],[FilterByDate],[StartDate],[EndDate],[FilterByMRP],[StartMRP],[EndMRP],[Date],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                            + " ('" + MainPage.strBranchCode + "','" + strIncOn + "','" + txtData.Text + "','" + strIncType + "' ," + dValue + " ,'" + chkDate.Checked.ToString() + "' ," + strStartDate + "," + strEndDate + ",'" + chkMRP.Checked.ToString() + "' ," + dStartMRP + "," + dEndMRP + ",DATEADD(MINUTE,30,DATEADD(hh,5,GETUTCDATE())) ,'" + MainPage.strLoginName + "','',0,0) end ";

            int _count = dba.ExecuteMyQuery(strQuery);
            if (_count > 0)
            {
                MessageBox.Show("Thank you ! Record submitted successfully  !", "Success", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                BindData();
                if (strOldIncentiveValue != "" && strOldIncentiveOn != "")
                {
                    strOldBranchCode = strOldIncentiveValue = strOldIncentiveOn = "";
                    rdoByBrand.Checked = true;
                    chkDate.Checked = chkMRP.Checked = false;
        ...
        {
            string strIncentiveOn = Convert.ToString(row.Cells["incentiveOn"].Value), strIncentiveValue = Convert.ToString(row.Cells["data"].Value), strBranchCode = Convert.ToString(row.Cells["branchCode"].Value);
            if (strIncentiveOn != "" && strIncentiveValue != "")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    string strQuery = " Delete from IncentiveDetails Where IncentiveOn='" + strIncentiveOn + "' and InentiveValue='" + strIncentiveValue + "' and BranchCode='" + strBranchCode + "' ";
                    int _count = dba.ExecuteMyQuery(strQuery);
                    if (_count > 0)
                    {
        ...
        private void EnableAllControls()
        {
            ...
            txtArvlDtTo.ReadOnly = txtArvlDtFrom.ReadOnly = txtToDate.ReadOnly = txtFromDate.ReadOnly = txtCoupanValidCount.ReadOnly = txtOfferAmt.ReadOnly = txtOfferDesc.ReadOnly = txtAmtFrom.ReadOnly = txtAmtTo.ReadOnly = txtQty.ReadOnly = txtFreeQty.ReadOnly = txtRewardCoupon.ReadOnly = txtDiscount.ReadOnly = txtOfferName.ReadOnly = false;
        }
        ...
        private void DisableAllControls()
        {
            ...
            txtArvlDtTo.ReadOnly = txtArvlDtFrom.ReadOnly = txtToDate.ReadOnly = txtFromDate.ReadOnly = txtCoupanValidCount.ReadOnly = txtOfferAmt.ReadOnly = txtOfferDesc.ReadOnly = txtAmtFrom.ReadOnly = txtAmtTo.ReadOnly = txtQty.ReadOnly = txtFreeQty.ReadOnly = txtRewardCoupon.ReadOnly = txtDiscount.ReadOnly = txtOfferName.ReadOnly = true;
        }
        ...
        private void txtOfferDt_Leave(object sender, EventArgs e)
        {
        ...
        private void DiscountAndIncentive_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
            {
        ...
        private void SetDataInControl(DataGridViewRow row)
        {
            strOldIncentiveOn = Convert.ToString(row.Cells["incentiveOn"].Value);
            SetIncentiveOn(strOldIncentiveOn);
            strOldIncentiveValue = txtData.Text = Convert.ToString(row.Cells["data"].Value);
            strOldBranchCode = Convert.ToString(row.Cells["branchCode"].Value);

            if (Convert.ToString(row.Cells["incentiveType"].Value) == "PER")
                rdoPer.Checked = true;
            else
                rdoValue.Checked = true;
            if (Convert.ToString(row.Cells["startDate"].Value) != "")
            {
                chkDate.Checked = true;
                txtFromDate.Text = Convert.ToString(row.Cells["startDate"].Value);
                txtToDate.Text = Convert.ToString(row.Cells["endDate"].Value);
            }
            double dToMRP = dba.ConvertObjectToDouble(row.Cells["endMRP"].Value);
            if (dToMRP > 0)
            {
                chkMRP.Checked = true;
                txtFromMRP.Text = Convert.ToString(row.Cells["startMRP"].Value);
                txtToMRP.Text = dToMRP.ToString("N2", MainPage.indianCurancy);
            }
            txtValue.Text = Convert.ToString(row.Cells["incentiveValue"].Value);
        }
