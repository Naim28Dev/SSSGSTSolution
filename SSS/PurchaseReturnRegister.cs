...
namespace SSS
{
    public partial class PurchaseReturnRegister : Form
    {
        DataBaseAccess dba;
        ...
        private void GetAdvanceSearchedRecord()
        {
            try
            {
                ...
                strQuery = " Select SR.ID,(SR.BillCode+' '+ CAST(SR.BillNo as varchar)) SRBillNo,Convert(varchar,Date,103)BDate,dbo.GetFullName(SalePartyID) SalesParty,SRBillNo as PurchaseSNo,dbo.GetFullName(PurchasePartyID) PurchaseParty,ItemName,Qty,Amount,freight,packing,SR.TaxFree,SRD.taxFree as STaxFree,totalAmt, (OtherSign+CAST(otherAmt as varchar)) as OtherAmt,NetDiscount as discountAmt,taxPer,CAST(GD.IGSTAmt as numeric(18,2))IGSTAmt,CAST(GD.CGSTAmt as numeric(18,2))CGSTAmt,CAST(GD.CGSTAmt as numeric(18,2)) SGSTAmt,TaxAmount as taxAmt,grossAmt,netAmt,createdBy,updatedBy,(PurchaseBillCode+' '+PurchaseBillNo)PBillNo,CONVERT(varchar,PurchaseBilLDate,103)PBillDate from PurchaseReturn SR inner join PurchaseReturnDetails SRD on SR.BillCode=SRD.BillCode and SR.BillNo=SRD.BillNo OUTER APPLY(Select (CASE WHEN GD.TaxType='LOCAL' then ROUND((SUM(GD.TaxAmount)/2),2) else 0 end) CGSTAmt,(CASE WHEN GD.TaxType='INTERSTATE' then ROUND(SUM(GD.TaxAmount),2) else 0 end) IGSTAmt from GSTDetails GD WHere BillType='PURCHASERETURN' and GD.BillCode=SR.BillCode and GD.BillNo=SR.BillNo Group by TaxType) GD Where [EntryType]!='CREDITNOTE' and  SR.BillCode!='' " + strSubQuery + " Order by SR.BillNo,SR.Date ";
        ...
        private string CreateQuery()
        {
            ...
            try
            {
                ...
                if(rdoByPurchaseSNo.Checked)
                    strQuery += " and EntryType ='BYSALERETURN' ";
                else if (rdoManual.Checked)
                    strQuery += " and EntryType ='MANUAL' ";
                else if (rdoAll.Checked)
                    strQuery += " and EntryType ='ALL' ";
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in table.Rows)
                        {
                            ...
                            if (strID != strOldID)
                            {
                                ...
                                row.Cells["purchaseBillNo"].Value = dr["PBillNo"];
                                row.Cells["purchaseBillDate"].Value = dr["PBillDate"];
                            ...
                            row.Cells["taxFree"].Value = dr["StaxFree"];
        ...
        private void dgrdGoods_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                string[] strReport = { "Exception occurred in Click Event of Data Grid View ", ex.Message };
                dba.CreateErrorReports(strReport);
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgrdDetails.Rows.Count > 0)
                {
                    ...
                    saveFileDialog.FileName = "Purchase_Return_Register";
        ...
        private void PurchaseReturnRegister_Load(object sender, EventArgs e)
        {
            try
            {
