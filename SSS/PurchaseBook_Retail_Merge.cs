...
namespace SSS
{
    public partial class PurchaseBook_Retail_Merge : Form
    {
        DataBaseAccess dba;
        string strLastSerialNo = "", strOldPartyName = "", _strPDFFilePath = "", _strBillType = "", _strSType = "";
        double dOldNetAmt = 0,dRowCount=0;
        bool qtyAdjustStatus = false, _bVariant1 = false, _bVariant2 = false, _bRoundTo5 = false, _bRoundToU5 = false, _bMUAfterDisc = false, _bMUAfterTax = false,_bMarginIncludeTax=false,_bSaleMRPFromPurMRP=false;

        SearchDataOther _objSearch;
        SearchData _objData;
        public PurchaseBook_Retail_Merge()
        {
            InitializeComponent();
        ...
        public PurchaseBook_Retail_Merge(string strSerialCode, string strSerialNo)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            SetCategory();
        ...
        private void GetStartupData(bool bStatus)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (Convert.ToString(row["Data8"]) != "")
                        chkMarginIncludeTax.Checked = _bMarginIncludeTax = Convert.ToBoolean(row["Data8"]);
                }
                ...
                if (ds.Tables.Count > 1)
                {
                    ...
                    if (dt != null)
                    {
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            txtTCSPer.Text = txtTCSAmt.Text = "0.0";
        ...
        {
            txtSplDisAmt.ReadOnly = txtNoOfPacks.ReadOnly = txtWeight.ReadOnly = txtSpclDisPer.ReadOnly = txtDiscAmt.ReadOnly = txtTaxFreeSign.ReadOnly = txtOtherPerSign.ReadOnly = txtDate.ReadOnly = txtInvoiceDate.ReadOnly = txtInvoiceNo.ReadOnly = txtRemark.ReadOnly = txtSign.ReadOnly = txtOtherAmt.ReadOnly = txtPackingAmt.ReadOnly = txtOtherPer.ReadOnly = txtTaxPer.ReadOnly = txtROSign.ReadOnly = txtRoundOff.ReadOnly =txtTaxFreeAmt.ReadOnly= txtTaxAmt.ReadOnly = true;
        ...
        {
            txtMode.Text = txtReceivedBy.Text = txtCountedBy.Text = txtBarcodedBy.Text = txtNoOfPacks.Text = txtWeight.Text = txtPurchaseRef.Text = txtImportData.Text= txtGSTNo.Text = txtStateName.Text = txtImportData.Text = txtTransport.Text = strOldPartyName = txtPurchaseParty.Text = txtTaxLedger.Text = txtAgentName.Text = txtTaxLedger.Text = txtRemark.Text = txtInvoiceNo.Text = txtGodown.Text =  txtLRNo.Text = txtmanufacturer.Text = "";
        ...
        {
            _strSType = "";
             txtPurchaseParty.Text =txtImportData.Text = txtTransport.Text = strOldPartyName = txtInvoiceNo.Text = txtGodown.Text = txtStockStatus.Text = txtLRNo.Text = txtmanufacturer.Text = "";
            ...
            try
            {
                if ((btnAdd.Text == "&Save" || btnEdit.Text == "&Update") && (Control.ModifierKeys & Keys.Control) != Keys.Control)
                {
                    ...
                    else if (e.ColumnIndex == 2)
                    {
                        ...
                        //dgrdDetails.CurrentRow.Cells["itemName"].Value = dgrdDetails.CurrentRow.Cells["variant1"].Value = dgrdDetails.CurrentRow.Cells["variant2"].Value = dgrdDetails.CurrentRow.Cells["variant3"].Value = dgrdDetails.CurrentRow.Cells["variant4"].Value = dgrdDetails.CurrentRow.Cells["variant5"].Value = dgrdDetails.CurrentRow.Cells["unitName"].Value = "";
                        //dgrdDetails.CurrentRow.Cells["taxPer"].Value = dgrdDetails.CurrentRow.Cells["taxAmount"].Value = "0.00";
                    ...
                    else if (e.ColumnIndex == 5)
                    {
                        ...
                        {
                            if (MainPage.bPurchaseSetWise)
                            {
                                _objSearch = new SearchDataOther("", "DESIGNNAME", "", "", "", "", "", "", Keys.Space, true, "");
                                _objSearch.ShowDialog();
                                GetAllDesignSizeColor(_objSearch, dgrdDetails.CurrentRow.Index, "");
                            }
                            else
                            {
                                _objSearch = new SearchDataOther("", "DESIGNNAMESINGLE", "", "", "", "", "", "", Keys.Space, true, "");
                                _objSearch.ShowDialog();
                                dgrdDetails.Rows[e.RowIndex].Cells["itemName"].Value = _objSearch.strSelectedData;
                            }
                        }
                        e.Cancel = true;
                    }
                        ...
                        if (strData != "")                       
                        {
                            ...
                            if (strAllItem.Length > 0)
                            {
                                ...
                                if (Convert.ToString(dgrdDetails.Rows[rowIndex].Cells["id"].Value) == "")
                                {
                                    ...
                                    SetUnitName(strAllItem[0],Convert.ToString(dgrdDetails.Rows[rowIndex].Cells["variant1"].Value),Convert.ToString(dgrdDetails.Rows[rowIndex].Cells["variant2"].Value), rowIndex, strCompanyCode);
                                }
        ...
        private void SetUnitName(string strDesignName,string strVariant1,string strVariant2, int rowIndex,string strCompanyCode)
        {
            if (strDesignName != "")
            {
            ...
            try
            {
                ...
                if (row != null)
                {
                    ...
                    if (Convert.ToString(row.Cells["itemName"].Value) != "")
                    {
                        ...
                        row.Cells["mrp"].Value = dba.ConvertObjToFormtdString(objValue);
                        if (dDisPer != 0)
                            dDisPer = dDisPer * -1;
                    }
                }
                ...
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                {
                    ...
                    if (e.KeyCode == Keys.Enter)
                    {
                        ...
                        if (IndexColmn < dgrdDetails.ColumnCount - 5)
                        {
                            ...
                            if (CurrentRow >= 0)
                            {
                                if (IndexColmn == 6 && _bVariant1)
                                    IndexColmn++;
                                if (IndexColmn == 7 && _bVariant2)
                                    IndexColmn++;
        ...
        private void CalculateBasicAmount()
        {
            try
            {
                double dBasicAmt = 0, _dMRP, dDisPer, dQty, dMRP, dRate, dAmt;
                string strDisStatus;
                foreach (DataGridViewRow row in dgrdDetails.Rows)
                {
                    dRate = 0;
        ...
        private void CalculateAllAmount()
        {
            try
            {
                ...
                foreach (DataGridViewRow rows in dgrdDetails.Rows)
                {
                    ...
                    else if(chkSaleMRPWithMRP.Checked) 
                    ...
                    if (!_bMUAfterTax && !_bMarginIncludeTax)
                        dSaleMRP += dGstAmt;

                    dSaleMRP = Math.Round(dSaleMRP, 0);
                    dSaleRate = (dSaleMRP * (100 - dSaleDis)) / 100.00;
                    dSaleRate = Math.Round(dSaleRate, 0);
            ...
            try
            {
                ...
                else if (ValidateControls() && CheckBillNoAndSuggest())
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        int count = 1;
                        if (!MainPage._bTaxStatus)
                            count = SavePurchaseRecord(ref strItemQuery);
                        if (count > 0)
                        {
                            count = SaveRecord(strItemQuery);
                            if (count <= 0)
        ...
        private int SaveRecord(string strItemQuery)
        {
            int _count = 0;
            try
            {
                string strInvoiceDate = "NULL", strLRDate = "NULL", strMNID = "",strGSTNo="";
        ...
        private int UpdateRecord(string strSubQuery)
        {
            ...
            try
            {
                string strInvoiceDate = "NULL", strLRDate="NULL",strMNID="", strTaxAccountID ="",strGSTNo="";
                DateTime strDate = dba.ConvertDateInExactFormat(txtDate.Text);
                if (txtInvoiceDate.Text.Length == 10)
                {
                    DateTime iDate = dba.ConvertDateInExactFormat(txtInvoiceDate.Text);
        ...
        {
            double dTaxAmt = 0;
            string _strTaxType = "", strHSNQuery,strHSNCode;
            try
            {
                ...
                {
                    strHSNCode = Convert.ToString(rows.Cells["hsnCode"].Value);
                    if (MainPage.bHSNWisePurchase && strHSNCode!="")                    
                        strHSNQuery = " Select Top 1 ItemName from Items _Im left Join ItemGroupMaster IGM on _IM.GroupName=IGM.GroupName  Where ItemName Like('%" + strHSNCode + "')  OR HSNCode in ('" + strHSNCode + "') ";
                    else
                        strHSNQuery = "'"+Convert.ToString(rows.Cells["itemName"].Value)+"' ";
                        ...
                        if (dgrdDetails.Rows.Count > 0)
                    {
                        ...
                        if (dRate > 0)
                        {
                            if (strQuery != "")
                                strQuery += " UNION ALL ";
                            strQuery += " Select '' as ID, (GM.Other + ' : ' + GM.HSNCode) as HSNCode," + dQty + " as Quantity,ROUND((((" + dAmt + " )*(100 - " + dDisStatus + "))/ 100.00),4)Amount,GM.TaxRate from Items _IM Outer APPLY (Select TOP 1 _IGM.Other, _IGM.HSNCode, (CASE WHEN _TC.ChangeTaxRate = 1 then (CASE WHEN _TC.GreaterORSmaller = '>' then(CASE WHEN((((((CASE WHEN '" + _strTaxType + "' = 'INCLUDED' then((" + dRate + " * 100) / (100 + TaxRate)) else " + dRate + " end))) * (CASE WHEN _TC.AmountType='NET PRICE' then ((100.00 - " + dDisStatus + ") / 100.00) else 1.00 end)))> _TC.ChangeAmt) then _TC.TaxChangeRateIGST else _TC.TaxRateIGST end) else (CASE WHEN((((((CASE WHEN '" + _strTaxType + "' = 'INCLUDED' then((" + dRate + "* 100) / (100 + TaxRate)) else " + dRate + " end))) * (CASE WHEN _TC.AmountType='NET PRICE' then ((100.00 - " + dDisStatus + ") / 100.00) else 1.00 end)))< _TC.ChangeAmt) then _TC.TaxChangeRateIGST else _TC.TaxRateIGST end) end) else _TC.TaxRateIGST end) TaxRate from ItemGroupMaster _IGM  left join TaxCategory _TC on _IGM.TaxCategoryName = _TC.CategoryName Where _IM.GroupName = _IGM.GroupName and _IGM.ParentGroup = '' ) as GM Where _IM.ItemName in (" + strHSNQuery + ") and " + dAmt + ">0  ";
                        }
                            ...
                            if (str.Length > 1)
                            {
                                ...
                                if (objSearch.strSelectedData != "")
                                {
                                    ...
                                    if (str.Length > 1)
                                    {
                                        ...
                                        txtPAmt.Text = ConvertObjectToDouble(str[1]).ToString("N2", MainPage.indianCurancy);
                                        CalculateAllAmount();
                                    }
                                    else
                                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                           ...
                           // txtOtherAmount.Text = Convert.ToString(_row["GAmt"]);
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                dgrdDetails.Rows[rowIndex].Cells["mrp"].Value = Math.Round(Convert.ToDouble(row["MRP"]), 4);//.ToString("N2", MainPage.indianCurancy);
                                dgrdDetails.Rows[rowIndex].Cells["rate"].Value = Math.Round(Convert.ToDouble(row["Rate"]), 4);//.ToString("N2", MainPage.indianCurancy);
                                dgrdDetails.Rows[rowIndex].Cells["amount"].Value = Convert.ToDouble(row["Amount"]).ToString("N2", MainPage.indianCurancy);
                             
                                dgrdDetails.Rows[rowIndex].Cells["unitName"].Value = row["UnitName"];
                                dgrdDetails.Rows[rowIndex].Cells["brandName"].Value = row["BrandName"];
                                dgrdDetails.Rows[rowIndex].Cells["styleName"].Value = row["DesignName"];
                                dgrdDetails.Rows[rowIndex].Cells["saleMargin"].Value = Math.Round(ConvertObjectToDouble(row["SaleMargin"]), 4);//.ToString("N4", MainPage.indianCurancy);
                ...
                else if (!strText.Contains("Less") && !strText.Replace(" ", "").ToUpper().Contains("CGST") && !strText.Replace(" ", "").ToUpper().Contains("SGST") && !strText.Replace(" ", "").ToUpper().Contains("IGST") && !strText.ToUpper().Contains("DISCOUNT") && !strText.ToUpper().Contains("JURISDICTION") && !strText.ToUpper().Contains("COMPUTER"))
                {
                    ...
                    if (strItem != "")
                    {
                        ...
                        if (strItem != "")
                        {
                            ...
                            SetUnitName(strItem,"","", _lineIndex - 1,MainPage.strDataBaseFile);
                        }
                        else
                        {
            ...
            for (; _index < _lines.Length - 1; _index++)
            {
                ...
                if (strItem != "")
                {
                    ...
                    if (strItem != "")
                    {
                        dgrdDetails.Rows[_lineIndex - 1].Cells["itemName"].Value = strItem;
                        GetPurchaseRate_Import(dgrdDetails.Rows[_lineIndex - 1]);
                        SetUnitName(strItem,strSize,strColor, _lineIndex - 1,MainPage.strDataBaseFile);
                    }
                    else
                    {
                        dgrdDetails.Rows[_lineIndex - 1].DefaultCellStyle.BackColor = Color.Tomato;
        ...
        private void txtSpclDisPer_KeyPress(object sender, KeyPressEventArgs e)
        {
            dba.KeyHandlerPoint(sender, e, 4);
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                string strData1 = chkVariant1.Checked.ToString(), strData2 = chkVariant2.Checked.ToString(), strData3 = chkRoundTo5.Checked.ToString(), strData4 = chkRoundToU5.Checked.ToString(), strData5 = chkMUAfterDisc.Checked.ToString(), strData6 = chkMuAfterTax.Checked.ToString(), strData7 = chkMarginIncludeTax.Checked.ToString(), strData8 = chkSaleMRPWithMRP.Checked.ToString(), strData9 = "0", strData10 = "0", strOther1 = "", strOther2 = "", strOther3 = "";
                ...
                if (_count > 0)
                {
                    ...
                    _bSaleMRPFromPurMRP = chkSaleMRPWithMRP.Checked;
        ...
        private int SavePurchaseRecord(ref string strItemQuery)
        {
            ...
            if (txtTaxLedger.Text.Contains("EXCLUDE"))
                _bInclude = false;
            string strBillNo = txtBillCode.Text + " " + txtBillNo.Text, _strPBillNo="";
            foreach (DataGridViewRow row in dgrdDetails.Rows)
            {
