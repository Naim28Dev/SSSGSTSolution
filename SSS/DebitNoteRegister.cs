...
namespace SSS
{
    public partial class DebitNoteRegister : Form
    {
        DataBaseAccess dba;
        public DebitNoteRegister()
        {
            InitializeComponent();
        ...
        private void GetAdvanceSearchedRecord()
        {
            try
            {
                ...
                strQuery = " Select SR.BillCode,SR.BIllNo, (CASE WHEN SaleBillNo>0 then (SaleBillCode+' '+CAST(SaleBillNo as varchar)) else '' end) as SaleBillNo,SR.ID,Convert(varchar,Date,103)BDate,dbo.GetFullName(SalePartyID) SalesParty,ItemName,Variant1,Amount,(OtherSign+CAST(otherAmt as varchar)) as OtherAmt,taxPer,CAST(GD.IGSTAmt as numeric(18,2))IGSTAmt,CAST(GD.CGSTAmt as numeric(18,2))CGSTAmt,CAST(GD.CGSTAmt as numeric(18,2)) SGSTAmt,TaxAmount as taxAmt,grossAmt,netAmt,createdBy,updatedBy from SaleReturn SR inner join SaleReturnDetails SRD on SR.BillCode=SRD.BillCode and SR.BillNo=SRD.BillNo OUTER APPLY(Select (CASE WHEN GD.TaxType='LOCAL' then ROUND((SUM(GD.TaxAmount)/2),2) else 0 end) CGSTAmt,(CASE WHEN GD.TaxType='INTERSTATE' then ROUND(SUM(GD.TaxAmount),2) else 0 end) IGSTAmt from GSTDetails GD WHere BillType='DEBITNOTE' and GD.BillCode=SR.BillCode and GD.BillNo=SR.BillNo Group by TaxType) GD Where [EntryType]='DEBITNOTE' "+strSubQuery+" Order by SR.BillNo,SR.Date ";
        ...
        private string CreateQuery()
        {
            ...
            try
            {
                ...
                if (txtSaleBillNo.Text != "")
                    strQuery += " and (SaleBillCode+' '+CAST(SaleBillNo as varchar)) Like('%" + txtSaleBillNo.Text + "%') ";
                if (txtRemark.Text != "")
        ...
        private void ShowDetails(string strAccount)
        {
            try
            {
                ...
                if (strAccount == "DEBITNOTE")
                {
                    string[] strNumber = strInvoiceNo.Split(' ');
                    if (strNumber.Length > 1)
                    {                       
                            DebitNote_Customer objSale = new DebitNote_Customer(strNumber[0], strNumber[1]);
                            objSale.FormBorderStyle = FormBorderStyle.FixedToolWindow;
                            objSale.ShowInTaskbar = true;
                            objSale.Show();                        
                    }
                }
                else if (strAccount == "SALES")
                {
                    string[] strNumber = strInvoiceNo.Split(' ');
                    if (strNumber.Length > 1)
                    {
                        SaleBook objGoodscumPurchase = new SaleBook(strNumber[0], strNumber[1]);
