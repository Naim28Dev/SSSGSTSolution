...
namespace SSS
{
    public partial class CartonTypeMaster : Form
    {
        DataBaseAccess dba;
        DataTable objTable;
        ...
        public CartonTypeMaster(bool chk,string strName)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            BindDataGrid();
        ...
        private void BindDataGrid()
        {
            try
            {
                objTable = dba.GetDataTable("Select * from CartoneType Order by Cartone");
                if (objTable != null)
                {
                    dgrdName.Rows.Clear();
                    if (objTable.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in objTable.Rows)
                        {
                            ...
                            dgrdName.Rows[rowIndex].Cells["itemName"].Value = dr["Cartone"];
        ...
        private void SaveRecord()
        {
            try
            {
               // string Insertquery = "Insert into UnitMaster (UnitName,FormalName,DecimalPoint) values('" + txtName.Text + "','" + txtFormalName.Text + "','"+txtDecimalPoint.Text+"')";
                int rd = dba.SaveNewCartoneType(txtName.Text);
                if (rd > 0)
                {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {    
                if (btnAdd.Text == "&Add" && btnEdit.Text == "&Edit" && lblId.Text!="")
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        int i = dba.ExecuteMyQuery("Delete from CartoneType Where ID=" + lblId.Text + "");
                        if (i > 0)
                        {
            ...
            try
            {
                ...
                if (lblId.Text != "" && row != null)
                {
                    if (Convert.ToString(row.Cells["id"].Value) == lblId.Text)
                    {
                        ...
                        int count = dba.UpdateCartoneType(txtName.Text, strOldName);
                        if (count > 0)
                        {
        ...
        {           
            try
            {
                if (txtName.Text != "")
                {
                    if (btnAdd.Text == "&Save")
                    {
                        DataTable MyTable = dba.GetDataTable("Select Cartone from CartoneType Where Cartone ='" + txtName.Text + "'");
                        if (MyTable.Rows.Count > 0)
                        {
                            lblMsg.Text = "Sorry ! Cartone Type : " + txtName.Text + " already exist ! ";
                    ...
                    {
                        DataTable MyTable = dba.GetDataTable("Select Cartone from CartoneType Where Cartone ='" + txtName.Text + "' and ID !=" + lblId.Text + "");
                        if (MyTable.Rows.Count > 0)
                        {
                            lblMsg.Text = "Sorry ! Cartone Type : " + txtName.Text + " already exist ! ";
                            lblMsg.ForeColor = Color.Red;
                ...
                {
                    lblMsg.Text = "Sorry ! Cartone Type can't be blank ! ";
                    lblMsg.ForeColor = Color.Red;
                    txtName.Focus();
                    return false;
                }
        ...
        private void SearchResult()
        {
            try
            {
                if (objTable != null)
                {
                    if (txtSearch.Text!="")
                    {
                        DataRow[] row = objTable.Select(String.Format("Cartone Like ('%" + txtSearch.Text + "%')"));
                        if (row.Length > 0)
                        {
