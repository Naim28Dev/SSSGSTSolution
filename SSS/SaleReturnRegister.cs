...
namespace SSS
{
    public partial class SaleReturnRegister : Form
    {
        DataBaseAccess dba;
        ...
        private void btnGo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chkDate.Checked && (txtFromDate.Text.Length != 10 || txtToDate.Text.Length != 10))
                    ...
                    MessageBox.Show(" Sorry ! Please fill serial no or uncheck serial no box ! ", "Serial no Required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                else if (chkSaleBillNo.Checked && (txtSaleBillNoFrom.Text == "" || txtSaleBillNoTo.Text == ""))
                    MessageBox.Show(" Sorry ! Please fill serial no or uncheck sale bill no box ! ", "Sale bill no Required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                else
                    GetAdvanceSearchedRecord();               
            }
            catch
        ...
        private void GetAdvanceSearchedRecord()
        {
            try
            {
                ...
                strQuery = " Select SR.ID,(SR.BillCode+' '+ CAST(SR.BillNo as varchar)) SRBillNo,Convert(varchar,Date,103)BDate,dbo.GetFullName(SalePartyID) SalesParty,ISNULL(dbo.GetFullName(SubPartyID),'SELF') SubParty,(SR.SaleBillCode+' '+CAST(SR.SaleBillNo as varchar)) SaleBillNo,PurchaseBillNo as PurchaseSNo,dbo.GetFullName(PurchasePartyID) PurchaseParty,ItemName,Qty,Amount,freight,packing,taxFree,totalAmt, (OtherSign+CAST(otherAmt as varchar)) as OtherAmt,NetDiscount as discountAmt,ServiceAmt,packingAmt,taxPer,CAST(GD.IGSTAmt as numeric(18,2))IGSTAmt,CAST(GD.CGSTAmt as numeric(18,2))CGSTAmt,CAST(GD.CGSTAmt as numeric(18,2)) SGSTAmt,TaxAmount as taxAmt,grossAmt,netAmt,PurchaseReturnStatus as  returnStatus,createdBy,updatedBy from SaleReturn SR inner join SaleReturnDetails SRD on SR.BillCode=SRD.BillCode and SR.BillNo=SRD.BillNo OUTER APPLY(Select (CASE WHEN GD.TaxType='LOCAL' then ROUND((SUM(GD.TaxAmount)/2),2) else 0 end) CGSTAmt,(CASE WHEN GD.TaxType='INTERSTATE' then ROUND(SUM(GD.TaxAmount),2) else 0 end) IGSTAmt from GSTDetails GD WHere BillType='SALERETURN' and GD.BillCode=SR.BillCode and GD.BillNo=SR.BillNo Group by TaxType) GD Where [EntryType]!='DEBITNOTE'  " + strSubQuery + " Order by SR.BillNo,SR.Date ";
            ...
            {
                string[] strReport = { "Exception occurred in Getting Advance Searched Record in Sale Return", ex.Message };
        ...
        private string CreateQuery()
        {
            ...
            try
            {
                ...
                if (chkSaleBillNo.Checked)
                    strQuery += " and (SR.SaleBillNo >= " + txtSaleBillNoFrom.Text + " and SR.SaleBillNo <=" + txtSaleBillNoTo.Text + ") ";
                ...
                if (txtQty.Text != "")                
                    strQuery += " and Qty =" + txtQty.Text + " ";                

                if (rdoClear.Checked)
                    strQuery += " and PurchaseReturnStatus =1 ";
                else if (rdoPending.Checked)
                    strQuery += " and PurchaseReturnStatus =0 ";
        ...
        private void BindDataWithGrid(DataTable table)
        {
            try
            {
                double dGrossAmt = 0, dNetAmt = 0, dTotalQty = 0;
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in table.Rows)
                        {
                            ...
                            if (strID != strOldID)
                            {
                                ...
                                row.Cells["saleBillNo"].Value = dr["saleBillNo"];
                                row.Cells["otherAmt"].Value = dr["otherAmt"];
                                row.Cells["discountAmt"].Value = dr["discountAmt"];
                                row.Cells["ServiceAmt"].Value = dr["ServiceAmt"];
                                row.Cells["packingAmt"].Value = dr["packingAmt"];
                            ...
                            row.Cells["purchaseSNo"].Value = dr["purchaseSNo"];
                            ...
                            row.Cells["taxFree"].Value = dr["taxFree"];
                            row.Cells["totalAmt"].Value = dr["totalAmt"];

                            dTotalQty += dba.ConvertObjectToDouble(dr["Qty"]);

                            if (Convert.ToBoolean(dr["returnStatus"]))
                            {
                                row.Cells["returnStatus"].Value = "CLEAR";
                                row.DefaultCellStyle.BackColor = Color.LightGreen;
                            }
                            else
                                row.Cells["returnStatus"].Value = "PENDING";
                ...
                lblTotalQty.Text = dTotalQty.ToString("N2", MainPage.indianCurancy);
            }
            catch (Exception ex)
            {
        ...
        private void chkSaleBillNo_CheckedChanged(object sender, EventArgs e)
        {
            txtSaleBillNoFrom.ReadOnly = txtSaleBillNoTo.ReadOnly = !chkSaleBillNo.Checked;
            txtSaleBillNoFrom.Text = txtSaleBillNoTo.Text = "";
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgrdDetails.Rows.Count > 0)
                {
                    ...
                    saveFileDialog.FileName = "Sale_Return_Register";
        ...
        private void SaleReturnRegister_Load(object sender, EventArgs e)
        {
            try
            {
