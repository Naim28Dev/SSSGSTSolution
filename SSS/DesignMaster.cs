...
namespace SSS
{
    public partial class DesignMaster : Form
    {
        DataBaseAccess dba;
        ...
        string strOldDesignName="", strOldCategory="", strOldUnitName="";
        ...
        public DesignMaster(string strSerialCode, string strSerialNo, bool _bDStatus)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            SetCategory();
        ...
        public DesignMaster(bool nStatus)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
        ...
        public DesignMaster(string strDesignName)
        {
            InitializeComponent();
            dba = new DataBaseAccess();
            SetCategory();
                ...
                if (ds.Tables.Count > 1)
                {
                    ...
                    if (dt != null)
                    {
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            txtCategory.Text = strOldCategory = Convert.ToString(row["Other"]);
        ...
        private void DesignMaster_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                ...
                else if (pnlImage.Visible)
                {
                    if (picdesign.Width == 720)
                    {
                        picdesign.Width = 360;
                        picdesign.Height = 305;
                        picdesign.Parent = panel5;
                    }
                    else
        ...
        private void dgrdDetails_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                {
                    if (Convert.ToString(dgrdDetails.CurrentRow.Cells["id"].Value) != "" && Convert.ToString(dgrdDetails.CurrentCell.Value) != "")
                    {
        ...
        private void dgrdDetails_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            try
            {
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                {
                    ...
                    else if (colIndex == 9 || colIndex == 10 || colIndex == 11 || colIndex== 13 || colIndex == 14 || colIndex == 15 || colIndex == 16 || colIndex == 17)
                    {
                        TextBox txtBox = (TextBox)e.Control;
                        if (txtBox != null)                        
        ...
        private void txtBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if(colIndex==14)
            {
             ...
             else if (colIndex == 9 || colIndex == 10 || colIndex == 11 || colIndex == 12 || colIndex == 13 || colIndex == 14 || colIndex == 16)
             {
                 dba.KeyHandlerPoint(sender, e, 2);
             }
        }
            ...
            try
            {
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                {
                    ...
                    {
                        {
                            ...
                            else if (Index == dgrdDetails.RowCount - 1)
                            {
                                ...
                                if ((strPRate != "" && strPRate != "0") || (strSRate != "" && strSRate != "0") || (strQty != "" && strQty != "0") || Convert.ToString(dgrdDetails.Rows[CurrentRow].Cells["category1"].Value) != "" || Convert.ToString(dgrdDetails.Rows[CurrentRow].Cells["category2"].Value) != "" || Convert.ToString(dgrdDetails.Rows[CurrentRow].Cells["category3"].Value) != "" || Convert.ToString(dgrdDetails.Rows[CurrentRow].Cells["category4"].Value) != "" || Convert.ToString(dgrdDetails.Rows[CurrentRow].Cells["category5"].Value) != "")
                                {
                                    ...
                                    dgrdDetails.Rows[dgrdDetails.RowCount - 1].Cells["barCodeID"].Value = dba.GetBarCode(txtSerialNo.Text, dgrdDetails.Rows.Count,"");
                                }
                                else
                                {
                                    if (btnAdd.Text == "&Save")
                    ...
                    else if (e.Modifiers == Keys.Control && e.KeyCode == Keys.D)
                    {
                        ...
                        dgrdDetails.Rows[_rowCount].Cells["barCodeID"].Value = "";// dgrdDetails.CurrentRow.Cells["barCodeID"].Value;
        ...
        private bool ValidateControls()
        {
           ...
           // {
            ...
            foreach (DataGridViewRow rows in dgrdDetails.Rows)
            {
                ...
                if ((strPRate == "" || strPRate == "0") && (strSRate == "" || strSRate == "0") && dOpeningQty == 0 && (Convert.ToString(rows.Cells["category1"].Value) == "" && Convert.ToString(rows.Cells["category2"].Value) == "" && Convert.ToString(rows.Cells["category3"].Value) == "" && Convert.ToString(rows.Cells["category4"].Value) == "" && Convert.ToString(rows.Cells["category5"].Value) == ""))
                {
                ...
                //else if (strBarCode=="" &&  dgrdDetails.Columns["barCodeID"].Visible)
                //{
                //    MessageBox.Show("Sorry ! Bar code can't be blank ! ", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                //    dgrdDetails.CurrentCell = rows.Cells["barCodeID"];
                //    dgrdDetails.Focus();
                //    return false;
                //}
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnAdd.Text == "&Add")
                {
                    ...
                    SetDesignSerialNo();
                    ClearAllText();
                    pnlSearch.Visible = pnlImage.Visible = false;                  
        ...
        {            
            ...
            string[] strFullParty;
            string strPurchasePartyID = "",strBarCode="",strUpdateStockQuery="", strInnerQuery="",strOuterQuery="";
            int _Index = 1;
            foreach (DataGridViewRow row in dgrdDetails.Rows)
            {
                ...
                strInnerQuery += " INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[DisPer],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                              + " (0,'" + txtSerialCode.Text + "',@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + ",'" + dSaleMRP + "'," + dDisPer + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate
                              + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',1,0)";
                //if (dOQty > 0)
                {
                    strInnerQuery += " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                  + " ('OPENING','" + txtSerialCode.Text + "',@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dOQty + "," + dORate + " ,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') ";
                }

                strUpdateStockQuery += " if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[DisPer]) VALUES "
                      + " (0,'" + txtSerialCode.Text + "',@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleMRP + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate + " "
                      + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0," + dDisPer + ") "
                      + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                      + " ('OPENING','" + txtSerialCode.Text + "',@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dOQty + "," + dORate + " ,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end else begin "
                      + " INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[DisPer]) VALUES "
                      + " (0,'" + txtSerialCode.Text + "',@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleMRP + "," + dSaleRate + "," + dReorder + ",0,0 "
                      + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0," + dDisPer + ") "
                      + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                      + " ('OPENING','" + txtSerialCode.Text + "',@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "',0,0,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end ";
            ...
            strQuery = strQuery + strInnerQuery + strOuterQuery + " end ";
        ...
        private string GetMappingQuery()
        {
            ...
            string[] strFullParty;
            string strPurchasePartyID = "", strBarCode = "", strUpdateStockQuery = "", strOuterQuery = "";
            int _Index = 1;
            foreach (DataGridViewRow row in dgrdDetails.Rows)
            {
                dMargin = dba.ConvertObjectToDouble(row.Cells["margin"].Value);
                ...
                if (strFullParty.Length > 1)
                    ...
                    strPurchasePartyID = "";
                strUpdateStockQuery += " if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                      + " (0,'" + txtSerialCode.Text + "',@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate + " "
                      ...
                      + " (0,'" + txtSerialCode.Text + "',@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleRate + "," + dReorder + ",0,0 "
                      + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0) "
                      + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                      + " ('OPENING','" + txtSerialCode.Text + "',@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "',0,0,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end ";
        ...
        private int UpdateDesignMaster(string strSubQuery,string strBarCodeQuery)
        {
            ...
            {
                DateTime strDate = dba.ConvertDateInExactFormat(txtDate.Text);
                string strItemType = "PURCHASE", strNetQuery = "", strOuterQuery = "", strMainQuery = "", strCodingType="";
                if (rdoForJournal.Checked)
                    strItemType = "JOURNAL";
                if (MainPage._bBarCodeStatus)
                {
                ...
                if (strOldCategory != txtCategory.Text)
                {
                    strQuery += "Update ItemStock Set IsWithoutStock=" + (txtCategory.Text == "WITHOUT STOCK" ? "1" : "0") + " Where ItemName='" + strOldDesignName + "' ";
                }
                if (strOldDesignName != txtItemName.Text)
                {
                    strQuery += " Update GoodsReceiveDetails Set ItemName='" + txtItemName.Text + "' Where ItemName='" + strOldDesignName + "' "
                           ...
                           + " Update ItemStock Set ItemName='" + txtItemName.Text + "' Where ItemName='" + strOldDesignName + "' ";
                }
                ...
                string[] strFullParty;
                string strPurchasePartyID = "",strBarCode="", strUpdateStockQuery="",strInnerQuery="",strOldBarCode="", strChangeDetailsUpdate="",strDBCode= MainPage.strDataBaseFile;
                int _index = 1;
                foreach (DataGridViewRow row in dgrdDetails.Rows)
                {
                    dMargin = dba.ConvertObjectToDouble(row.Cells["margin"].Value);
                    ...
                    if (strID != "" && strID != "0")
                    {
                        strInnerQuery += " UPDATE [dbo].[ItemSecondary] SET [PurchasePartyID]='" + strPurchasePartyID + "',[Variant1]='" + row.Cells["category1"].Value + "',[Variant2]='" + row.Cells["category2"].Value + "',[Variant3]='" + row.Cells["category3"].Value + "',[Variant4]='" + row.Cells["category4"].Value + "',[Variant5]='" + row.Cells["category5"].Value + "',[PurchaseRate]=" + dPurchaseRate + ",[Description]='" + strBarCode + "',"
                        ...
                        strNetQuery += " UPDATE [dbo].[ItemSecondary] SET [PurchasePartyID]='" + strPurchasePartyID + "',[Variant1]='" + row.Cells["category1"].Value + "',[Variant2]='" + row.Cells["category2"].Value + "',[Variant3]='" + row.Cells["category3"].Value + "',[Variant4]='" + row.Cells["category4"].Value + "',[Variant5]='" + row.Cells["category5"].Value + "',[PurchaseRate]=" + dPurchaseRate + ",[Description]='" + strBarCode + "',"
                                    + " [Margin]=" + dMargin + ",[SaleMRP]=" + dOldSmrp + ",[SaleRate]=" + dSaleRate + ",[Reorder]=" + dReorder + ",[OpeningQty]=" + dOQty + ",[OpeningRate]=" + dORate + ",[ActiveStatus]='" + Convert.ToBoolean(row.Cells["Chk"].Value) + "',[GodownName]='" + row.Cells["godownName"].Value + "',[UpdatedBy]='" + MainPage.strLoginName + "' Where [RemoteID]=" + strID + " and [BillCode]='" + txtSerialCode.Text + "' and [BillNo]=" + txtSerialNo.Text + " ";
                        ...
                        if (strOldBarCode != "")
                        {
                            ...
                            strUpdateStockQuery += " if exists (Select BillCode from [dbo].[ItemSecondary] Where [Description]='" + strOldBarCode + "' and [Description] Like('" + strDBCode + "%') and [BillCode]=@SerialCode and [BillNo]=@SerialNo) begin if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin UPDATE [dbo].[ItemSecondary] SET [PurchasePartyID]='" + strPurchasePartyID + "',[Variant1]='" + row.Cells["category1"].Value + "',[Variant2]='" + row.Cells["category2"].Value + "',[Variant3]='" + row.Cells["category3"].Value + "',[Variant4]='" + row.Cells["category4"].Value + "',[Variant5]='" + row.Cells["category5"].Value + "',[PurchaseRate]=" + dPurchaseRate + ",[Description]='" + strBarCode + "',"
                                                ...
                                                + " UPDATE [dbo].[ItemSecondary] SET [PurchasePartyID]='" + strPurchasePartyID + "',[Variant1]='" + row.Cells["category1"].Value + "',[Variant2]='" + row.Cells["category2"].Value + "',[Variant3]='" + row.Cells["category3"].Value + "',[Variant4]='" + row.Cells["category4"].Value + "',[Variant5]='" + row.Cells["category5"].Value + "',[PurchaseRate]=" + dPurchaseRate + ",[Description]='" + strBarCode + "',"
                                                + " [Margin]=" + dMargin + ",[SaleMRP]=" + dSaleMRP + ",[DisPer]=" + dDisPer + ",[SaleRate]=" + dSaleRate + ",[Reorder]=" + dReorder + ",[ActiveStatus]='" + Convert.ToBoolean(row.Cells["Chk"].Value) + "',[GodownName]='" + row.Cells["godownName"].Value + "',[UpdatedBy]='" + MainPage.strLoginName + "',[UpdateStatus]=1 Where [Description]='" + strOldBarCode + "' and [Description] Like('" + strDBCode + "%') and [BillCode]=@SerialCode and [BillNo]=@SerialNo "
                                                + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                                + " ('OPENING',@SerialCode,@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "',0,0,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end end ";
                    ...
                    else
                    {
                        ...
                        strInnerQuery += " INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[DisPer],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                                 + " (0,'" + txtSerialCode.Text + "'," + txtSerialNo.Text + ",'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleMRP + "," + dDisPer + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate
                                 + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',1,0)";

                        strUpdateStockQuery += " if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[DisPer],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                                            + " (0,@SerialCode,@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleMRP + "," + dDisPer + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate + " "
                                            + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0) "
                                            + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                            + " ('OPENING',@SerialCode,@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dOQty + "," + dORate + " ,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end else begin "
                                            + " INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[DisPer],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                                            + " (0,@SerialCode,@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleMRP + "," + dDisPer + "," + dSaleRate + "," + dReorder + ",0,0 "
                                            + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0) "
                                            + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                            + " ('OPENING',@SerialCode,@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "',0,0,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end ";
                ...
                strQuery = " Delete from [dbo].[StockMaster] Where BillType='OPENING' and BillCode='" + txtSerialCode.Text + "' and BillNo=" + txtSerialNo.Text + " "
                        + strQuery + strSubQuery + strInnerQuery + strOuterQuery;
        ...
        private string GetUpdateQuery()
        {
            ...
            try
            {
                ...
                double dORate = 0, dOQty = 0, dMargin = 0, dSaleMRP = 0, dDisPer = 0, dSaleRate = 0, dPurchaseRate = 0, dReorder = 0, dTOpeningQty = 0;
                string[] strFullParty;
                string strPurchasePartyID = "", strBarCode = "", strOldBarCode = "", strDBCode = MainPage.strDataBaseFile;
                int _index = 1;
                foreach (DataGridViewRow row in dgrdDetails.Rows)
                {
                    dMargin = dba.ConvertObjectToDouble(row.Cells["margin"].Value);
                    ...
                    strUpdateStockQuery += " if exists (Select BillCode from [dbo].[ItemSecondary] Where [Description]='" + strOldBarCode + "' and [Description] Like('" + strDBCode + "%') and [BillCode]=@SerialCode and [BillNo]=@SerialNo) begin if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin UPDATE [dbo].[ItemSecondary] SET [PurchasePartyID]='" + strPurchasePartyID + "',[Variant1]='" + row.Cells["category1"].Value + "',[Variant2]='" + row.Cells["category2"].Value + "',[Variant3]='" + row.Cells["category3"].Value + "',[Variant4]='" + row.Cells["category4"].Value + "',[Variant5]='" + row.Cells["category5"].Value + "',[PurchaseRate]=" + dPurchaseRate + ",[Description]='" + strBarCode + "',"
                                        ...
                                        + " [Margin]=" + dMargin + ",[SaleMRP]=" + dSaleMRP + ",[DisPer]=" + dDisPer + ",[SaleRate]=" + dSaleRate + ",[Reorder]=" + dReorder + ",[ActiveStatus]='" + Convert.ToBoolean(row.Cells["Chk"].Value) + "',[GodownName]='" + row.Cells["godownName"].Value + "',[UpdatedBy]='" + MainPage.strLoginName + "',[UpdateStatus]=1 Where [Description]='" + strOldBarCode + "' and [Description] Like('" + strDBCode + "%') and [BillCode]=@SerialCode and [BillNo]=@SerialNo "
                                        + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                        + " ('OPENING',@SerialCode,@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "',0,0,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end end else begin "
                                    + " if exists (Select CompanyName from CompanyDetails Where TAXEnabled=0) begin INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
                                    + " (0,@SerialCode,@SerialNo,'" + strPurchasePartyID + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dPurchaseRate + "," + dMargin + "," + dSaleRate + "," + dReorder + "," + dOQty + "," + dORate + " "
                                    + ",'" + Convert.ToBoolean(row.Cells["Chk"].Value) + "','" + row.Cells["godownName"].Value + "','" + strBarCode + "','" + MainPage.strLoginName + "','',0,0) "
                                    + " INSERT INTO [dbo].[StockMaster] ([BillType],[BillCode],[BillNo],[ItemName],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[Qty],[Rate],[GodownName],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus],[MRP],[Date],[BarCode],[BrandName]) VALUES "
                                    + " ('OPENING',@SerialCode,@SerialNo, '" + txtItemName.Text + "','" + row.Cells["category1"].Value + "','" + row.Cells["category2"].Value + "','" + row.Cells["category3"].Value + "','" + row.Cells["category4"].Value + "','" + row.Cells["category5"].Value + "'," + dOQty + "," + dORate + " ,'" + row.Cells["godownName"].Value + "','" + MainPage.strLoginName + "','',1,0," + dORate + ",'" + strDate.ToString("MM/dd/yyyy") + "','" + strBarCode + "','" + txtBrandName.Text + "') end else begin "
                                    + " INSERT INTO [dbo].[ItemSecondary] ([RemoteID],[BillCode],[BillNo],[PurchasePartyID],[Variant1],[Variant2],[Variant3],[Variant4],[Variant5],[PurchaseRate],[Margin],[SaleMRP],[DisPer],[SaleRate],[Reorder],[OpeningQty],[OpeningRate],[ActiveStatus],[GodownName],[Description],[CreatedBy],[UpdatedBy],[InsertStatus],[UpdateStatus]) VALUES "
            ...
            if (MainPage._bBarCodeStatus)
            {
                try
                {
                    if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                    {
                        ...
                        object objValue = DataBaseAccess.ExecuteMyScalar("Select BillNo from ItemSecondary Where Description!='' and Description in (" + strAllBarCode + ") and BillCode in ('" + txtSerialCode.Text + "') and BillNo not in (" + txtSerialNo.Text + ") ");
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnAdd.Text == "&Save" || btnEdit.Text == "&Update")
                {
                    DialogResult dr = MessageBox.Show("Are you sure you want to Clear Design Image ? ", "Clear Design Image", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (dr == DialogResult.Yes)
                    {
                        picdesign.ImageLocation = MainPage.strServerPath + "/DesignImage/BlankImage.jpg";
                    }
                }
            }
            catch
        ...
        private void btnBrowse_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {                   
                    picdesign.ImageLocation = objDialog.FileName;
                }
            }
            catch
            {
        ...
        private void SaveNewImage()
        {
            try
            {
                string strImagePath = picdesign.ImageLocation;
                if (strImagePath.Contains("BlankImage.jpg"))
                {
                    string strPath = MainPage.strServerPath + "/DesignImage/" + txtItemName.Text;
                    if (Directory.Exists(strPath))
                    {
                        string[] strAllFiles = Directory.GetFiles(strPath);
                ...
                else if (!strImagePath.Contains("DesignImage"))
                {
                    string strExtension = Path.GetExtension(strImagePath), strNewPath = MainPage.strServerPath + "/DesignImage/" + txtItemName.Text, strNewImage = strNewPath + "/" + txtItemName.Text + strExtension;
                    Directory.CreateDirectory(strNewPath);
                    if (File.Exists(strNewImage))
        ...
        private void ShowSavedImage()
        {
            string strDefaultPath = MainPage.strNetImagePath + "/BlankImage.jpg";
            try
            {
                string strPath = MainPage.strServerPath + "/DesignImage/" + txtItemName.Text;
                // string strPath = MainPage.strNetImagePath + "/" + txtItemName.Text + ".jpg";
                if (Directory.Exists(strPath))
                {
                    string[] strFiles = Directory.GetFiles(strPath);
                    if (strFiles.Length > 0)
                    {
                        picdesign.ImageLocation = strPath;
                    }
                    else
                    {
                        picdesign.ImageLocation = strDefaultPath;
                    }
                }
                else
                    picdesign.ImageLocation = strDefaultPath;
        ...
        private void picdesign_Click(object sender, EventArgs e)
        {
            try
            {
                if (picdesign.Width == 360)
                {
                    picdesign.Width = 720;
                    picdesign.Height = 620;
                    picdesign.Parent = this;
                    picdesign.BringToFront();
                    //Point objPoint = new Point(395, 107);
                    //picdesign.Location = objPoint;
                }
                else
                {
                    picdesign.Width = 360;
                    picdesign.Height = 305;
                    picdesign.Parent = panel5;
                    //Point objPoint = new Point(600, 229);
                    //picdesign.Location = objPoint;
                }
            }
            catch
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string strImagePath = picdesign.ImageLocation;
                if (!strImagePath.Contains("http") && !strImagePath.Contains("ftp"))
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        UploadImagesToFTP(strImagePath);
                    }
                }
                else
                    MessageBox.Show("Sorry ! Please browse design image", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch
            {
            }
        ...
        private void UploadImagesToFTP(string strImagePath)
        {

            string strExtension = Path.GetExtension(strImagePath), strNewImage = txtItemName.Text + strExtension;
            DataBaseAccess.DeleteFTPImage(strNewImage);
            bool bStatus = DataBaseAccess.UploadDesignImageToFTP(strNewImage, strImagePath);
            if (bStatus)
            {
                MessageBox.Show("Thank you ! Image uploaded successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
        }
        ...
        private DataTable CreateDataTable()
        {
            ...
            foreach (DataGridViewRow row in dgrdDetails.Rows)
            {
                if (Convert.ToBoolean(row.Cells["printCheck"].Value))
                {
                    strVariant1 = strVariant2 = "";
                    dSaleRate = dba.ConvertObjectToDouble(row.Cells["saleRate"].Value);
                    if (dSaleRate > 0)
                    {
                        strVariant1 = Convert.ToString(row.Cells["category1"].Value);

                        if (Convert.ToString(row.Cells["category2"].Value) != "")
                        {
                            if (strVariant1 != "")
                                strVariant1 += " (";
                            strVariant1 += Convert.ToString(row.Cells["category2"].Value);
                        ...
                        _row["ItemName"] = txtItemName.Text;
                        _row["Variant1"] = strVariant1;
                        _row["Qty"] = txtQtyRatio.Text + " " + txtUnitName.Text;
                        _row["SetNo"] = "SETNO11" + dSaleRate + "00";
                        _row["BarCode"] = "*" + row.Cells["barCodeID"].Value + "*";
                        _row["Date"] = MainPage.currentDate.ToString("yyMMdd");
                        _dt.Rows.Add(_row);
                    }
                }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            //            Reporting.BarCodeReport objReport = new Reporting.BarCodeReport();
            //            objReport.SetDataSource(dt);
            //            Reporting.ShowReport objShow = new Reporting.ShowReport("Bar Code");
